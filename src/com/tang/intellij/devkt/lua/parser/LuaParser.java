...
package com.tang.intellij.devkt.lua.parser;
...
import static com.tang.intellij.devkt.lua.psi.LuaTypes.*;

@SuppressWarnings({"SimplifiableIfStatement", "UnusedAssignment"})
public class LuaParser implements PsiParser, LightPsiParser {

	public static final TokenSet[] EXTENDS_SETS_ = new TokenSet[]{create_token_set_(EXPR_LIST,
			VAR_LIST), create_token_set_(NAME_DEF, PARAM_NAME_DEF), create_token_set_(ARGS,
			LIST_ARGS,
			SINGLE_ARG), create_token_set_(BINARY_EXPR,
			CALL_EXPR,
			CLOSURE_EXPR,
			EXPR,
			INDEX_EXPR,
			...
			PAREN_EXPR,
			TABLE_EXPR,
			UNARY_EXPR), create_token_set_(ASSIGN_STAT,
			BREAK_STAT,
			DO_STAT,
			EMPTY_STAT,
			EXPR_STAT,
			...
			RETURN_STAT,
			WHILE_STAT),};
	final static Parser checkFuncPrefix_parser_ = (b, l) -> checkFuncPrefix(b, l + 1);
	final static Parser parList_recover_parser_ = (b, l) -> parList_recover(b, l + 1);
	final static Parser stat_recover_parser_ = (b, l) -> stat_recover(b, l + 1);
	final static Parser tableField_recover_parser_ = (b, l) -> tableField_recover(b, l + 1);
