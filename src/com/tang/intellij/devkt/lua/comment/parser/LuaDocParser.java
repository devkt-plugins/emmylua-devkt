...
package com.tang.intellij.devkt.lua.comment.parser;
...
import static com.tang.intellij.devkt.lua.comment.psi.LuaDocTypes.*;
import static com.tang.intellij.devkt.lua.psi.LuaParserUtil.*;
import static org.jetbrains.kotlin.com.intellij.lang.PsiBuilder.Marker;

@SuppressWarnings({"SimplifiableIfStatement", "UnusedAssignment"})
public class LuaDocParser implements PsiParser, LightPsiParser {

	public static final TokenSet[] EXTENDS_SETS_ = new TokenSet[]{create_token_set_(ARR_TY,
			FUNCTION_TY,
			GENERAL_TY,
			GENERIC_TY,
			PAR_TY,
			...
			TY,
			UNION_TY),};
	final static Parser after_dash_recover_parser_ = (b, l) -> after_dash_recover(b, l + 1);
	...
	// {
	// }
	private static boolean class_name_ref_1(PsiBuilder b, int l) {
		return true;
	}
	...
	// ('|' ty)+ {
	// }
	private static boolean union_ty_0(PsiBuilder b, int l) {
		if (!recursion_guard_(b, l, "union_ty_0")) return false;
		boolean r;
		Marker m = enter_section_(b);
		r = union_ty_0_0(b, l + 1);
		r = r && union_ty_0_1(b, l + 1);
		exit_section_(b, m, null, r);
		return r;
	}

	// ('|' ty)+
	private static boolean union_ty_0_0(PsiBuilder b, int l) {
		if (!recursion_guard_(b, l, "union_ty_0_0")) return false;
		boolean r;
		Marker m = enter_section_(b);
		r = union_ty_0_0_0(b, l + 1);
		int c = current_position_(b);
		while (r) {
			if (!union_ty_0_0_0(b, l + 1)) break;
			if (!empty_element_parsed_guard_(b, "union_ty_0_0", c)) break;
			c = current_position_(b);
		}
		exit_section_(b, m, null, r);
		return r;
	...
	// '|' ty
	private static boolean union_ty_0_0_0(PsiBuilder b, int l) {
		if (!recursion_guard_(b, l, "union_ty_0_0_0")) return false;
		boolean r;
		Marker m = enter_section_(b);
	...
	// {
	// }
	private static boolean union_ty_0_1(PsiBuilder b, int l) {
		return true;
	}
	...
	// {
	// }
	private static boolean table_ty_1(PsiBuilder b, int l) {
		return true;
	}
	...
	// '[]' {
	// }
	private static boolean arr_ty_0(PsiBuilder b, int l) {
		if (!recursion_guard_(b, l, "arr_ty_0")) return false;
		boolean r;
		Marker m = enter_section_(b);
		r = consumeTokenSmart(b, ARR);
		r = r && arr_ty_0_1(b, l + 1);
		exit_section_(b, m, null, r);
		return r;
	}
	...
	// {
	// }
	private static boolean arr_ty_0_1(PsiBuilder b, int l) {
		return true;
	}
	...
	// {
	// }
	private static boolean general_ty_1(PsiBuilder b, int l) {
		return true;
	}
